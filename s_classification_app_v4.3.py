...
class ClassificationBackend:
    ...
    def create_area_pie_chart(self, area_stats, figsize=(10, 8)):
        """创建面积饼图"""
        if not area_stats:
            fig, ax = plt.subplots(figsize=figsize)
        ...
        wedges, texts, autotexts = ax.pie(areas, labels=labels, autopct='%1.1f%%',
        ...
        for label, stats in area_stats.items():
            table_data.append([label, f"{stats['pixel_count']:,}", f"{stats['area_ha']:.2f}", f"{stats['percentage']:.1f}%"])
...
def display_result_preview():
    ...
    try:
        ...
        if selected_classifier_code in st.session_state.area_statistics:
            ...
            area_df = pd.DataFrame([
                {
                    ...
                    '面积(公顷)': f"{stats['area_ha']:.2f}",
                    '百分比': f"{stats['percentage']:.1f}%"
                }
            ...
            # 显示饼图
            pie_fig = backend.create_area_pie_chart(area_stats)
            st.pyplot(pie_fig)
